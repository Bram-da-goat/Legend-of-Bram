...
function cast() {
  ...
  window.setTimeout(() => {
    ...
    save()
    render()
    if (['rare', 'epic', 'legendary'].includes(catchFish.rarity)) launchRareCelebration(catchFish)
  }, 900 + Math.random() * 1200)
}

function launchRareCelebration(catchFish) {
  const celebration = document.createElement('div')
  celebration.className = `celebration ${catchFish.rarity}`
  celebration.innerHTML = `<div class="catch-banner"><span>✦ RARE CATCH ✦</span><strong>${catchFish.name}</strong><small>${catchFish.rarity.toUpperCase()} · +${catchFish.value} COINS</small></div>`
  for (let i = 0; i < 54; i += 1) {
    ...
    piece.style.background = ['#ffd45e', '#ff7fa9', '#83e5ff', '#a9ff9a', '#d894ff'][i % 5]
    celebration.append(piece)
  }
  document.body.append(celebration)
  window.setTimeout(() => celebration.remove(), 2900)
}
    ...
    <main class="fishing-app">
      ...
      <section class="game-layout">
        <section class="water-card" style="--water:${spot.color};--sky:${spot.sky}">
          <div class="scene" id="scene"><div class="sun"></div><div class="cloud cloud-one"></div><div class="cloud cloud-two"></div><div class="hills hills-one"></div><div class="hills hills-two"></div><div class="water"><span class="water-shine"></span><span class="ripple one"></span><span class="ripple two"></span><span class="fish-shadow"></span><span class="cast-line ${fishing ? 'casting' : ''}"></span><span class="bobber ${fishing ? 'casting' : ''}">🔴</span><span class="cast-splash ${fishing ? 'casting' : ''}"></span></div><div class="dock">🪵</div><div class="angler">🎣</div></div>
...
effectsStyle.textContent = `
  .scene:before{content:'';position:absolute;inset:0;background:linear-gradient(135deg,#fff8 0%,transparent 28%,#fff2 65%,transparent 80%);mix-blend-mode:soft-light;pointer-events:none}.cloud{position:absolute;width:145px;height:28px;border-radius:99px;background:#ffffffa8;filter:blur(1px);opacity:.7;animation:cloudDrift 20s linear infinite}.cloud:before,.cloud:after{content:'';position:absolute;border-radius:50%;background:inherit}.cloud:before{width:54px;height:54px;left:22px;bottom:0}.cloud:after{width:38px;height:38px;right:24px;bottom:0}.cloud-one{left:7%;top:15%}.cloud-two{right:17%;top:26%;transform:scale(.72);animation-delay:-9s}.water{overflow:hidden}.water:after{content:'';position:absolute;inset:0;background:linear-gradient(100deg,transparent 15%,#ffffff1c 30%,transparent 45%);animation:waterGlint 6s linear infinite}.water-shine{position:absolute;left:55%;top:15%;width:240px;height:80px;border-radius:50%;background:#fdf6bd36;filter:blur(9px);animation:shinePulse 3.5s ease-in-out infinite}.fish-shadow{position:absolute;left:57%;top:58%;width:34px;height:13px;border-radius:50%;background:#0d415a80;transform:rotate(-12deg);animation:fishSwim 4s ease-in-out infinite}.cast-line{position:absolute;left:24%;top:-36%;height:75%;width:2px;transform-origin:top;background:linear-gradient(#f8ead7,#cfc3b2);opacity:0;z-index:2}.cast-line.casting{opacity:1;animation:castLine 1s ease-out forwards}.bobber{z-index:3}.bobber.casting{animation:castBobber 1s cubic-bezier(.16,.84,.31,1) forwards, bob .65s 1s infinite alternate}.cast-splash{position:absolute;left:55%;top:49%;width:15px;height:15px;border:3px solid #eafaff;border-radius:50%;opacity:0}.cast-splash.casting{animation:splash 1.35s .6s ease-out forwards}.celebration{position:fixed;z-index:50;inset:0;pointer-events:none;overflow:hidden;background:radial-gradient(circle at center,#ffffff24,transparent 43%);animation:celebrationFade 2.9s forwards}.catch-banner{position:absolute;left:50%;top:17%;transform:translateX(-50%) scale(.7);min-width:min(430px,86vw);padding:18px 25px;border:2px solid #ffe282;border-radius:18px;background:linear-gradient(135deg,#263c70f2,#563872f2);box-shadow:0 0 42px #ffe28299;text-align:center;animation:bannerPop .55s cubic-bezier(.18,.89,.32,1.3) forwards}.celebration.epic .catch-banner{background:linear-gradient(135deg,#48276f,#9b4f9b)}.celebration.legendary .catch-banner{background:linear-gradient(135deg,#6f4314,#b87e20)}.catch-banner span,.catch-banner small{display:block;color:#ffe48b;font-size:.74rem;font-weight:900;letter-spacing:.15em}.catch-banner strong{display:block;margin:5px 0;font:clamp(1.8rem,5vw,3.1rem) Georgia,serif;color:white;text-shadow:0 3px 3px #0007}.confetti{position:absolute;top:-12px;width:10px;height:18px;border-radius:2px;animation:confettiFall 2.7s var(--delay) cubic-bezier(.2,.6,.4,1) forwards}@keyframes cloudDrift{to{transform:translateX(420px)}}@keyframes waterGlint{from{transform:translateX(-110%)}to{transform:translateX(150%)}}@keyframes shinePulse{50%{transform:scale(1.2);opacity:.45}}@keyframes fishSwim{50%{transform:translate(-100px,21px) rotate(8deg)}}@keyframes castLine{0%{transform:rotate(-48deg) scaleY(.15)}60%{transform:rotate(20deg) scaleY(1)}100%{transform:rotate(11deg) scaleY(1)}}@keyframes castBobber{0%{transform:translate(-230px,-160px) rotate(-160deg)}70%{transform:translate(0,0) rotate(14deg)}100%{transform:translate(0,0)}}@keyframes splash{0%{opacity:0;transform:scale(.2)}30%{opacity:1}100%{opacity:0;transform:scale(8)}}@keyframes bannerPop{to{transform:translateX(-50%) scale(1)}}@keyframes confettiFall{to{transform:translate(var(--drift),110vh) rotate(850deg);opacity:0}}@keyframes celebrationFade{80%{opacity:1}100%{opacity:0}}
`
document.head.append(effectsStyle)
